...
The quorums are a group of constant communicated worker nodes that verify the workers produced information before broadcasting to the network.

A quorum is the only authorized member of the network to broadcast blocks, this way we can guarantee there are no malicious workers on the network.
...
2. (work-in-progress) Rhea (written in Rust)
